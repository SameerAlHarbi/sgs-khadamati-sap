...
exports.getTypes = async (lang = "A") => {
  ...
  try {
    ...
    const results = await sapClient.call("ZHR_MANDATE_VALUES", {
      IM_LANGU: lang,
    });
    ...
    return results["T_TYPE"].map((result) =>
      modelMapper.mapDelegationTypeDTO(result)
    );
  } catch (error) {
    throw error;
  }
...
exports.calcAmount = async (delegationRequest) => {
  try {
    const sapParams = {
      ...
      IM_FOOD:
        delegationRequest.costEndurance.toUpperCase() === "NON" ? "X" : " ",
      IM_HOUSE:
        delegationRequest.costEndurance.toUpperCase() !== "FULL" ? "X" : " ",
      IM_ESGROUP: "3",
    };
    ...
    const sapClient = await sapPool.acquire();
    const results = await sapClient.call(
      "ZHR_MANDATE_PAYMENT_CALCULATE",
      sapParams
    );

    if (
      !results ||
      !results["T_MANDATE_PAY"] ||
      results["T_MANDATE_PAY"].length < 1
    ) {
      return undefined;
    }
...
exports.getAllocation = async (employeeId, sapId) => {
  try {
    ...
    const sapClient = await sapPool.acquire();
    const results = await sapClient.call("ZHR_MANDATE_EARMARKED_CHECK", {
      IM_PERNR: employeeId,
      IM_MADNO: sapId,
    });
